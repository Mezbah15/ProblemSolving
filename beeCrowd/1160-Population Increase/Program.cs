...
for (int i = 1; i <= testCase; i++)
{
    ...
    double PA = double.Parse(input[0]); //100
    double PB = double.Parse(input[1]); //150
    double G1 = double.Parse(input[2]); // 1
    double G2 = double.Parse(input[3]); // 0

    int count = 0;
    while (true)
    {
        double popA = (PA * (G1 / 100.00)); //1
        double floorPopA = Math.Floor(popA);
        double popB = (PB * (G2 / 100.00));//0
        double floorPopB = Math.Floor(popB);
